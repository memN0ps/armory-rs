//! # AskTGS BOF
//!
//! Requests a Kerberos service ticket via raw TGS-REQ using an existing
//! TGT. Builds an AP-REQ with encrypted authenticator, sends to KDC
//! port 88, parses TGS-REP, and outputs a base64-encoded .kirbi.
//!
//! ## MITRE ATT&CK
...
fn main(args: *mut u8, len: usize) {
    println!("[*] Action: Ask TGS\n");

    if len == 0 {
        eprintln!("[X] /ticket:BASE64 /service:SPN required");
        return;
    }
    ...
    let tgs_req = match build_tgs_req(&api, &kirbi, service, &domain) {
        Some(r) => r,
        None => {
    ...
    if let Some(ticket) = handle_tgs_rep(&api, &response, &kirbi) {
        println!("[+] TGS request successful!");
        println!("[*] base64(ticket.kirbi):\n\n{}\n", ticket);
        if ptt {
            println!("[*] /ptt: use the ptt BOF to import this ticket");
        }
    }
}
