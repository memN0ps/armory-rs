...
//! - `/targetuser:USER` - Target user (optional, requires privileges)
//! - `/targetdomain:DOMAIN` - Target domain (optional)
...
fn main(args: *mut u8, len: usize) {
    ...
    if len == 0 { eprintln!("[X] /ticket:BASE64 /new:PASSWORD required"); return; }
    ...
    let new_pw = match get_param(&params, "/new:") { Some(p) => p, None => { eprintln!("[X] /new:PASSWORD must be supplied!"); return; } };
    ...
    match handle_tgs_rep(&api, &response, &kirbi) {
        Some(ticket) => {
            println!("[+] Password change - got kadmin/changepw ticket!");
