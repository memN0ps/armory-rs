...
fn find_ctx<'a>(asn: &'a Asn, tag: u32) -> Option<&'a Asn> {
    for child in &asn.children {
        if child.class == 2 && child.tag == tag {
            return Some(child);
        }
    }
    None
...
fn parse_principal_name(asn: &Asn) -> PrincipalName {
    let mut pn = PrincipalName { name_type: 0, names: Vec::new() };
...
fn parse_krb_cred_info(asn: &Asn) -> KrbCredInfo {
    let mut info = KrbCredInfo {
        key_type: 0, prealm: String::new(), pname: PrincipalName { name_type: 0, names: Vec::new() },
        flags: 0, starttime: DateTime { year: 0, month: 0, day: 0, hour: 0, minute: 0, second: 0 },
        endtime: DateTime { year: 0, month: 0, day: 0, hour: 0, minute: 0, second: 0 },
        renew_till: DateTime { year: 0, month: 0, day: 0, hour: 0, minute: 0, second: 0 },
        srealm: String::new(),
        sname: PrincipalName { name_type: 0, names: Vec::new() },
    };

    for child in &asn.children {
        if child.class != 2 { continue; }
        match child.tag {
            0 => {
            ...
            1 => { if let Some(c) = child.children.first() { info.prealm = get_string(c); } }
            2 => { if let Some(c) = child.children.first() { info.pname = parse_principal_name(c); } }
            3 => { if let Some(c) = child.children.first() { info.flags = get_bit_flags(c); } }
            6 => { if let Some(c) = child.children.first() { info.starttime = parse_generalized_time(c); } }
            7 => { if let Some(c) = child.children.first() { info.endtime = parse_generalized_time(c); } }
            8 => { if let Some(c) = child.children.first() { info.renew_till = parse_generalized_time(c); } }
            9 => { if let Some(c) = child.children.first() { info.srealm = get_string(c); } }
            10 => { if let Some(c) = child.children.first() { info.sname = parse_principal_name(c); } }
            _ => {}
        }
    }
    info
...
fn base64_decode(input: &str) -> Option<Vec<u8>> {
    ...
    while i < input.len() {
        ...
        if output.len() < out_len { output.push(((triple >> 16) & 0xFF) as u8); }
        if output.len() < out_len { output.push(((triple >> 8) & 0xFF) as u8); }
        if output.len() < out_len { output.push((triple & 0xFF) as u8); }
    }
    Some(output)
}
...
fn describe_ticket(data: &[u8]) {
    let root = match parse_asn(data) {
        Some(r) => r,
        None => { eprintln!("[X] Failed to parse ASN.1 structure"); return; }
    };

    let body = if root.class == 1 && root.tag == 22 {
        if let Some(seq) = root.children.first() { seq } else { &root }
    } else {
        &root
    };
    ...
    let enc_part_ctx = match find_ctx(body, 3) {
        Some(e) => e,
        None => { eprintln!("[X] No enc-part found in KRB-CRED"); return; }
    };

    let enc_data_seq = match enc_part_ctx.children.first() {
        Some(s) => s,
        None => { eprintln!("[X] Invalid enc-part structure"); return; }
    };

    let cipher_ctx = match find_ctx(enc_data_seq, 2) {
        Some(c) => c,
        None => { eprintln!("[X] No cipher in enc-part"); return; }
    };

    let cipher_bytes = match cipher_ctx.children.first() {
        Some(c) => &c.value,
        None => { eprintln!("[X] Invalid cipher"); return; }
    };

    let enc_cred_part = match parse_asn(cipher_bytes) {
        Some(r) => r,
        None => { eprintln!("[X] Failed to parse EncKrbCredPart"); return; }
    };

    let inner = if enc_cred_part.class == 1 {
        if let Some(seq) = enc_cred_part.children.first() { seq } else { &enc_cred_part }
    } else {
        &enc_cred_part
    };
    ...
    let ticket_info_ctx = match find_ctx(inner, 0) {
        Some(t) => t,
        None => { eprintln!("[X] No ticket-info in EncKrbCredPart"); return; }
    };

    let ticket_info_seq = match ticket_info_ctx.children.first() {
        Some(s) => s,
        None => { eprintln!("[X] Invalid ticket-info"); return; }
    };

    let first_info = match ticket_info_seq.children.first() {
        Some(i) => i,
        None => { eprintln!("[X] No KrbCredInfo entries"); return; }
    };

    let info = parse_krb_cred_info(first_info);

    println!("  ServiceName              :  {}", format_principal(&info.sname));
    println!("  ServiceRealm             :  {}", info.srealm);
    println!("  UserName                 :  {}", format_principal(&info.pname));
    println!("  UserRealm                :  {}", info.prealm);
    let st = &info.starttime;
    let et = &info.endtime;
    let rt = &info.renew_till;
    println!("  StartTime (UTC)          :  {:02}/{:02}/{:04} {:02}:{:02}:{:02}", st.day, st.month, st.year, st.hour, st.minute, st.second);
    println!("  EndTime (UTC)            :  {:02}/{:02}/{:04} {:02}:{:02}:{:02}", et.day, et.month, et.year, et.hour, et.minute, et.second);
    println!("  RenewTill (UTC)          :  {:02}/{:02}/{:04} {:02}:{:02}:{:02}", rt.day, rt.month, rt.year, rt.hour, rt.minute, rt.second);
    println!("  Flags                    :  {}", flags_to_string(info.flags));
    println!("  KeyType                  :  {}", etype_name(info.key_type));
}
...
fn main(args: *mut u8, len: usize) {
    ...
    let ticket_b64 = match get_param(&params, "/ticket:") {
        Some(t) => t,
        None => { eprintln!("[X] You must supply a /ticket!"); return; }
    };

    let ticket_bytes = match base64_decode(ticket_b64) {
        Some(b) if !b.is_empty() => b,
        _ => { eprintln!("[X] Failed to decode base64 ticket"); return; }
    };
