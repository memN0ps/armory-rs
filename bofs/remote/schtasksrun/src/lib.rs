...
//!
//! Immediately runs a scheduled task on a local or remote host by invoking
//! `schtasks /run /tn <name>` via `CreateProcessA` and capturing the output
...
fn main(args: *mut u8, len: usize) {
    ...
    let mut cmd = format!("schtasks /run /tn \"{}\"", taskname);
