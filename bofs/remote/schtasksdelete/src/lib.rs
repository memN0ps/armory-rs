//! # Scheduled Tasks Delete BOF
//!
//! Deletes a scheduled task on a local or remote host by invoking
//! `schtasks /delete /tn <name> /f` via `CreateProcessA` and capturing
//! the output through an anonymous pipe.
//! ## MITRE ATT&CK
...
fn main(args: *mut u8, len: usize) {
    ...
    let mut cmd = format!("schtasks /delete /tn \"{}\" /f", taskname);
    if !hostname.is_empty() {
