...
//!
//! Stops a running scheduled task on a local or remote host by invoking
//! `schtasks /end /tn <name>` via `CreateProcessA` and capturing the output
...
fn main(args: *mut u8, len: usize) {
    ...
    println!("schtasksstop:");
    println!("  hostname: {}", if hostname.is_empty() { "(local)" } else { &hostname });
    println!("  taskname: {}", taskname);

    let mut cmd = format!("schtasks /end /tn \"{}\"", taskname);
