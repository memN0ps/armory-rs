//! # UxSubclassInfo Injection BOF
//!
//! Injects shellcode into a remote process using the UxSubclassInfo
//! window subclass injection technique. This variant manipulates the
//! `UxSubclassInfo` property of a target window to redirect execution
//! flow by overwriting the subclass callback pointer with the address
//! of injected shellcode.
//!
//! `VirtualAllocEx` -> `WriteProcessMemory` -> `VirtualProtectEx` ->
//! UxSubclassInfo-based technique.
//! ## MITRE ATT&CK
...
fn main(args: *mut u8, len: usize) {
    ...
    println!("uxsubclassinfo injection:");
    println!("  pid:      {}", pid);
    ...
    println!("SUCCESS - uxsubclassinfo injection variant, remote thread created.");
