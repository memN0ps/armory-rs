...
impl Object {
    pub fn enumerate<F>(&self, mut visit: F) -> Result<u32, WmiError>
    where
        F: FnMut(&str, &Value),
    {
        unsafe {
            type BeginEnumeration = unsafe extern "system" fn(*mut c_void, i32) -> HResult;
            type NextProperty = unsafe extern "system" fn(
                *mut c_void,
                i32,
                *mut BstrRaw,
                *mut Variant,
            ...
            type EndEnumeration = unsafe extern "system" fn(*mut c_void) -> HResult;

            let begin: BeginEnumeration =
                core::mem::transmute(vtable_entry(self.object.as_ptr(), 8));
            let next: NextProperty = core::mem::transmute(vtable_entry(self.object.as_ptr(), 9));
            let end: EndEnumeration = core::mem::transmute(vtable_entry(self.object.as_ptr(), 10));

            let result = begin(self.object.as_ptr(), WBEM_FLAG_NONSYSTEM_ONLY);
            if result < 0 {
                return Err(WmiError {
                    stage: "IWbemClassObject::BeginEnumeration",
                    code: result,
                });
            }
            ...
            while count < 64 {
                ...
                if result < 0 {
                    failure = Some(WmiError {
                        stage: "IWbemClassObject::Next",
                        code: result,
                    });
                    break;
                }

                let name_value = bstr_to_string(name);
                let owned = value_to_owned(&value);
                if let Some(name_value) = name_value.as_deref() {
                    visit(name_value, &owned);
                }
            ...
            let end_result = end(self.object.as_ptr());
            if let Some(error) = failure {
                return Err(error);
            }
            if end_result < 0 {
                return Err(WmiError {
                    stage: "IWbemClassObject::EndEnumeration",
                    code: end_result,
                });
            }
            Ok(count)
