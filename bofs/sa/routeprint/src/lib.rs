...
use windows_sys::Win32::NetworkManagement::IpHelper::{
    GetAdaptersInfo, GetIpForwardTable, IP_ADAPTER_INFO, MIB_IPFORWARDTABLE, MIB_IPFORWARDROW,
};

fn ip_str(a: u32) -> alloc::string::String {
    format!("{}.{}.{}.{}", a & 0xFF, (a >> 8) & 0xFF, (a >> 16) & 0xFF, (a >> 24) & 0xFF)
}
...
fn main() {
    unsafe {
        ...
        if GetAdaptersInfo(core::ptr::null_mut(), &mut adapt_len) != ERROR_BUFFER_OVERFLOW {
            eprintln!("Failed to get adapter buffer size"); return;
        }
        ...
        if fwd_len == 0 { eprintln!("Failed to get forward table size"); return; }
        let mut fwd_buf = vec![0u8; fwd_len as usize];
        let fwd_table = fwd_buf.as_mut_ptr() as *mut MIB_IPFORWARDTABLE;

        if GetAdaptersInfo(adapters, &mut adapt_len) != 0 { eprintln!("GetAdaptersInfo failed"); return; }
        if GetIpForwardTable(fwd_table, &mut fwd_len, 1) != 0 { eprintln!("GetIpForwardTable failed"); return; }
        ...
        while !cur.is_null() {
            let a = &*cur;
            println!("0x{} ........................... {}", a.Index, from_cstr(&a.Description));
            cur = a.Next;
        }
        println!("===========================================================================");
        ...
        println!("{:<27}{:<17}{:<14}{:<11}{:<10}", "Network Destination", "Netmask", "Gateway", "Interface", "Metric");
        ...
        for i in 0..num {
            ...
            println!("{:>17}{:>17}{:>17}{:>16}{:>9}",
                ip_str(r.dwForwardDest), ip_str(r.dwForwardMask), ip_str(r.dwForwardNextHop),
