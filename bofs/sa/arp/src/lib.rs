...
use windows_sys::Win32::NetworkManagement::IpHelper::{GetIpNetTable, MIB_IPNETTABLE, MIB_IPNETROW_LH};

fn arp_type_str(t: u32) -> &'static str {
    match t { 1 => "other", 2 => "invalid", 3 => "dynamic", 4 => "static", _ => "unknown" }
}
...
fn main() {
    unsafe {
        ...
        if size == 0 { println!("No ARP entries found."); return; }

        let mut buffer = vec![0u8; size as usize];
        let table = buffer.as_mut_ptr() as *mut MIB_IPNETTABLE;
        if GetIpNetTable(table, &mut size, 1) != 0 { eprintln!("GetIpNetTable failed"); return; }
        ...
        for i in 0..num {
            ...
            if last_if != row.dwIndex {
                ...
                println!("{:<24}{:<24}{:<24}", "Internet Address", "Physical Address", "Type");
            }
            ...
            println!("{:<24}{:<24}{:<24}", ip, mac, arp_type_str(row.Anonymous.dwType));
        }
    }
}
