//! # Scheduled Tasks Enumerate BOF
//!
//! Enumerates all scheduled tasks on a local or remote host by invoking
//! `schtasks /query /fo list` via `CreateProcessA` and capturing the output
...
fn main(args: *mut u8, len: usize) {
    ...
    println!("schtasksenum:");
    println!("  hostname: {}", if hostname.is_empty() { "(local)" } else { &hostname });

    let cmd = if hostname.is_empty() {
        String::from("schtasks /query /fo list")
    } else {
        format!("schtasks /query /fo list /s {}", hostname)
    };
