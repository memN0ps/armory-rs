...
fn main(args: *mut u8, len: usize) {
    ...
    println!("{{\"host\":\"{}\",\"groups\":[", if hostname.is_empty() { "localhost" } else { &hostname });
    ...
    unsafe {
        ...
        let status = NetLocalGroupEnum(server_ptr, 1, &mut buf, MAX_PREFERRED_LENGTH, &mut read, &mut total, &mut resume);
        if status == 0 {
